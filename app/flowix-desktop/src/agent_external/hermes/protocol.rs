...
pub fn response_result(message: &Value, id: u64) -> Option<Result<&Value, String>> {
    ...
    if let Some(error) = message.get("error") {
        ...
        return Some(Err(format!(
            "{human_message}\nHermes JSON-RPC error: {envelope}"
        )));
    }
...
/// update.
///
/// hermes ACP puts the real tool name in `title` at the initial `tool_call`
...
/// `rawInput` is a non-empty JSON object ── the shape hermes ACP uses to
...
pub fn chunks_from_message(
    ...
    match update.get("sessionUpdate").and_then(Value::as_str) {
        ...
        Some("tool_call_update") if !is_terminal_tool_status(update) => {
            // hermes sends the actual tool arguments on `in_progress` updates
...
mod tests {
    ...
    fn json_rpc_errors_preserve_code_and_provider_data() {
        let message = json!({
            ...
            "error": {
                ...
                "data": { "status": 429, "request_id": "req-hermes" }
            }
        });
        ...
        assert!(error.contains("req-hermes"));
    }
    ...
    #[test]
    fn maps_hermes_tool_kind_and_camel_case_file_path() {
        let value = json!({
            "jsonrpc": "2.0",
            "method": "session/update",
            "params": {
    ...
    fn bash_pending_tool_call_keeps_plain_tool_name() {
        let value = tool_update(
            ...
            &[
                ...
                ("title", json!("bash")),
                ("status", json!("pending")),
                ("rawInput", json!({ "cwd": "/tmp/hermes/probe-project" })),
            ],
        );
        assert!(matches!(
            chunks(&value).as_slice(),
    ...
    fn read_tool_input_arrives_on_in_progress_update() {
        ...
        let in_progress = tool_update(
            ...
            &[
                ...
                (
                    "rawInput",
                    json!({
                        "filePath": "/private/tmp/hermes/probe-project/README.md"
                    }),
                ),
            ],
        );
        ...
        assert!(matches!(
            ...
            [AgentChunk::ToolCall { name, input, .. }]
                if name == "read"
                    && input.get("filePath").and_then(Value::as_str)
                        == Some("/private/tmp/hermes/probe-project/README.md")
        ));
    }
