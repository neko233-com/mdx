        ...
        {items.map((plugin) => (
          <div key={plugin.manifest.id} className="flex items-center justify-between rounded-lg border border-[var(--divider)] bg-[var(--card)] p-3">
            ...
            <div className="flex items-center gap-1">
              ...
              {!plugin.isSystem && <Button variant="ghost" size="icon" disabled={busy} onClick={() => void uninstall(plugin)} title="卸载插件" className="text-[var(--muted-foreground)] hover:text-[var(--destructive)]"><Trash2 className="h-4 w-4" /></Button>}
            </div>
          </div>
        ))}
      </div>
